...
void I2SInit(void);
//static void codec_init(void);  /* TO USE EXTRENAL MIC REMOVE COMMENT */
void HPF_init(void);
...
#endif //#ifdef ENABLE_SILENCE_DETECTION
        {
            ...
            if (ai85Counter >= SAMPLE_SIZE) {
                ...
                if (!ret || out_class == NUM_OUTPUTS - 1) {
                    ...
                    if (strcmp(keywords[out_class], "GO") == 0){
                        MXC_GPIO_OutSet(gpio_out2.port, gpio_out2.mask); //11 - GO
                        MXC_GPIO_OutSet(gpio_out1.port, gpio_out1.mask);
                    }
                    else if (strcmp(keywords[out_class], "STOP") == 0){
                        MXC_GPIO_OutClr(gpio_out1.port, gpio_out1.mask); //00 - STOP
                        MXC_GPIO_OutClr(gpio_out2.port, gpio_out2.mask);
                    }
                    else if (strcmp(keywords[out_class], "RIGHT") == 0){
                        MXC_GPIO_OutClr(gpio_out1.port, gpio_out1.mask); //01 - RIGHT
                        MXC_GPIO_OutSet(gpio_out2.port, gpio_out2.mask);
                    }
                    else if (strcmp(keywords[out_class], "LEFT") == 0){
                        MXC_GPIO_OutSet(gpio_out1.port, gpio_out1.mask); //10 - LEFT
                        MXC_GPIO_OutClr(gpio_out2.port, gpio_out2.mask);
                    }
