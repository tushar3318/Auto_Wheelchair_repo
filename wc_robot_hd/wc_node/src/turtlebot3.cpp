...
Wc::Wc(const std::string & usb_port)
: Node("wc_node", rclcpp::NodeOptions().use_intra_process_comms(true))
{
  RCLCPP_INFO(get_logger(), "Init Wc Node Main");
...
Wc::Wheels * Wc::get_wheels()
{
  return &wheels_;
}

Wc::Motors * Wc::get_motors()
{
  return &motors_;
}

void Wc::init_dynamixel_sdk_wrapper(const std::string & usb_port)
{
...
void Wc::publish_timer(const std::chrono::milliseconds timeout)
{
...
void Wc::heartbeat_timer(const std::chrono::milliseconds timeout)
{
...
void Wc::parameter_event_callback()
{
